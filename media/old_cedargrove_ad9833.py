...
class AD9833:
    ...
    def __init__(
        ...
        self._cs = digitalio.DigitalInOut(
            getattr(board, select)
        )  # derive chip select pin
        from adafruit_bus_device.spi_device import (
            SPIDevice,
        )  # initiate SPI device manager
    ...
    def update_freq(self, freq=440):  # update freq register value
        ...
        if self._freq_reg == 0:
            self._freq_reg = 1  # indicate inactive register
        else:
            self._freq_reg = 0
    ...
    def update_phase(self, ph=0):  # update freq register value
        ...
        if self._phase_reg == 0:
            self._phase_reg = 1  # indicate inactive register
        else:
            self._phase_reg = 0
    ...
    def update_control_reg(self):
        ...
        if self._pause:
            control_reg |= 0x0080  # disable master clock bit
